...
function filterByCondition(arr, condition) {
  const filteredArr = arr.filter((item) => condition(item));
  return filteredArr;
...
function createArray(...elements) {
  if (elements.length > 0) {
    ...
    console.log("Помилка: Елементи не передано");
    return [];
  }
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  const reversedArr = combinedArr.reverse();
